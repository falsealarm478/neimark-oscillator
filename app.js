...
let noticeMessage='';
const C={green:'#c1ed83',orange:'#f3ab71',purple:'#b7a2eb',grid:'#35495c',axis:'#96adc2',muted:'#d4dfeb',bg:'#15212d'};
let sim,playing=false,stopped=false,last=0,extent=1.6,flash=0;
...
function reset(){stopped=false;sim=new Neimark.Oscillator(params());extent=Math.max(1.5,sim.a*1.4,Math.abs(sim.v0)*1.5);flash=0;$('notice').hidden=true;for(const [k,v] of Object.entries(params()))$(k+'Value').value=v.toFixed(k==='delta'?3:2);$('q').textContent=Math.exp(Math.PI*sim.delta).toFixed(3);draw();}
function setPlaying(value){if(value&&stopped)return;playing=value;$('play').textContent=t(playing?'Ⅱ Пауза':'▶ Продолжить');draw();}
...
$('start').onclick=()=>{reset();setPlaying(true);};
$('stop').onclick=()=>{setPlaying(false);stopped=true;draw();};
$('step').onclick=()=>{if(stopped)return;setPlaying(false);advance(Math.PI);draw();};
...
document.addEventListener('keydown',e=>{if(!$('laboratory').hidden&&e.code==='Space'&&!['INPUT','SELECT','BUTTON','A'].includes(document.activeElement.tagName)&&!$('guide').open){e.preventDefault();setPlaying(!playing);}});
...
function canvas(id){const el=$(id),r=el.getBoundingClientRect(),dpr=Math.min(devicePixelRatio||1,2),w=r.width,h=r.height;if(el.width!==Math.round(w*dpr)||el.height!==Math.round(h*dpr)){el.width=Math.round(w*dpr);el.height=Math.round(h*dpr);}const g=el.getContext('2d');g.setTransform(dpr,0,0,dpr,0,0);g.clearRect(0,0,w,h);g.font='14px Segoe UI, Arial';g.lineJoin='round';g.lineCap='round';return {g,w,h};}
function line(g,x1,y1,x2,y2,color,width=1.3,dash=[]){g.beginPath();g.strokeStyle=color;g.lineWidth=width;g.setLineDash(dash);g.moveTo(x1,y1);g.lineTo(x2,y2);g.stroke();g.setLineDash([]);}
...
function axes(c,xmin,xmax,ymin,ymax,xlabel,ylabel){
  const {g,w,h}=c,L=60,R=w-24,T=32,B=h-46;
  ...
  text(g,xlabel,R,B+40,C.muted,'right');text(g,ylabel,L,18,C.muted);
  return {...c,X,Y,L,R,T,B};
}
function clip(p,fn){p.g.save();p.g.beginPath();p.g.rect(p.L,p.T,p.R-p.L,p.B-p.T);p.g.clip();fn();p.g.restore();}
function path(p,points,getX,getY,color=C.green){clip(p,()=>{const g=p.g;g.strokeStyle=color;g.lineWidth=2.5;g.beginPath();let first=true;for(const v of points){const x=p.X(getX(v)),y=p.Y(getY(v));if(first||v.jump){g.moveTo(x,y);first=false;}else g.lineTo(x,y);}g.stroke();});}
...
function drawOscillator(){
  const {g,w,h}=canvas('oscillator'),cx=w/2,cy=Math.min(62,h-52),left=32,right=w-32,scale=(right-left)*.36/extent,px=cx+sim.x*scale;
  ...
  arrow(g,px,cy-25,Math.max(-55,Math.min(55,sim.v/extent*55)),color);text(g,'v',px+64<right?px+64:px-64,cy-24,color,'center');
...
function draw(){if(!sim||$('laboratory').hidden)return;let peak=Math.max(1,sim.a,Math.abs(sim.v),Math.abs(sim.x));for(const p of sim.points)peak=Math.max(peak,Math.abs(p.x),Math.abs(p.v));for(const e of sim.events)peak=Math.max(peak,Math.abs(e.prev),Math.abs(e.after));extent=Math.max(extent,Math.ceil(peak*1.15*4)/4);drawOscillator();drawPhase();drawTimeline();drawMap();$('time').textContent=sim.t.toFixed(2);$('position').textContent=sim.x.toFixed(3);$('velocity').textContent=sim.v.toFixed(3);$('impacts').textContent=sim.hits;$('samples').textContent=I18n.language==='en'?`Crossings: ${sim.n} · pairs: ${sim.events.length}`:`Пересечения: ${sim.n} · пары: ${sim.events.length}`;$('status').textContent=t(stopped?'● ОСТАНОВЛЕНО':flash>.15?'● УДАР':playing?'● ДВИЖЕНИЕ':'● ПАУЗА');$('play').textContent=t(playing?'Ⅱ Пауза':'▶ Продолжить');if(!$('notice').hidden)$('notice').textContent=t(noticeMessage);$('play').disabled=stopped;$('step').disabled=stopped;$('stop').disabled=stopped;$('status').className='live'+(flash>.15?' impact':'');}
function frame(now){const dt=last?Math.min((now-last)/1000,.05):0;last=now;if(playing&&!$('laboratory').hidden&&!document.hidden&&!$('guide').open)advance(dt*Number($('speed').value));flash=Math.max(0,flash-dt*1.8);draw();requestAnimationFrame(frame);}
